...
void APokemon::MultiSetPokemonLocation_Implementation()
{
	if (HasAuthority())
	{
		SetActorLocation(FVector(-110.f, 678.f, 10.f));
	}
	else
	{
		SetActorLocation(FVector(-50.f, 88.f, 10.f));
	}
}

void APokemon::ServerSetPokemonLocation_Implementation()
{
	MultiSetPokemonLocation_Implementation();
}
...
void APokemon::OnRep_SetHealthBar()
{
	...
	if (!OwnedTrainer->oppoTrainer->CurrentPokemon->StatusWidget)	return;
...
void APokemon::Skill(ESkill Skill)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("APokemon::Skill >> ServerSkill() call"));
...
{
	UE_LOG(LogTemp, Warning, TEXT("APokemon::MultiSkill_Implementation"));
	...
	case ESkill::SkillWater:
		...
		//스킬 위력에 따른 데미지 계산, 물대포(위력 : 50/ 명중률 : 100)
		AttackDamage(120.f, OwnedTrainer->oppoTrainer->CurrentPokemon);
	...
	case ESkill::SkillFire:
		...
		{
			UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), FireThrowParticle, myLoc);
			UE_LOG(LogTemp, Warning, TEXT("ok"));
		}
		else
		{
...
{
	UE_LOG(LogTemp, Warning, TEXT("APokemon::ServerSkill_Implementation"));
	MultiSkill(skill, _myloc, _oppoloc);
}
